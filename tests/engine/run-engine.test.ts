...
describe("runEngine", () => {
  ...
  it("excludes exhausted no-contact markers from the needs_contact cohort", async () => {
    ...
    const exhaustedButNamed = await upsertPractice(t.db, {
      name: "Exhausted But Named Clinic",
      geoKey: "austin-tx",
      city: "Austin",
      state: "TX",
      websiteUrl: "https://exhausted-but-named.example",
    });
    await tagVertical(t.db, exhaustedButNamed.id, "dermatology");
    await attachSignal(t.db, {
      practiceId: exhaustedButNamed.id,
      kind: "phone_complaints",
      sourceUrl: "https://maps.example.com/exhausted-but-named",
      detectedAt: NOW,
      expiresAt: computeExpiresAt("phone_complaints", NOW),
      signalSource: "test",
    });
